...
if __name__ == '__main__':
    ...
    # 35 Epoch
    model = YOLO(r'C:\projects\CNNDLAT3\Final\dog_breed_detection\yolov12n.pt-v1.0\weights\best.pt')

    # Send model to GPU explicitly (optional, usually not needed)
    model.to('cuda' if torch.cuda.is_available() else 'cpu')

    metrics = model.val(data=r'C:\projects\CNNDLAT3\Final\detection\dataset.yaml', split='test', device=0)  # 0 = first GPU
